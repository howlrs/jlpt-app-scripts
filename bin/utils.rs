...
【多様性要件（重要）】
8. 問題文のシチュエーション（場面・主語・動詞）をバッチごとに変えること。
   同一セッションで「学校」「家庭」「買い物」「交通」「仕事」「天気」「食事」「趣味」など、
   最低でも5種類以上の異なる日常シーンを使い分けること。
9. 同じ選択肢セット（4語の組み合わせ）を繰り返し出題しないこと。
   特にN4・N5では助詞や基本動詞の選択肢が限られがちだが、問題文の文脈で多様性を出すこと。
10. 同じ正解語を2回以上連続で配置しないこと（同じ知識点を別問題で問う場合は、選択肢セットを変えること）。

【書式要件（重要：ユーザ画面に直接表示される）】
11. 穴埋めの空欄は必ず「（　　）」(全角括弧+全角スペース2個) で統一すること。
    「（）」「( )」「（ ）」「（　）」「＿＿＿」「____」等の揺れは禁止。
    ただし並び替え問題 (category=9 文の組み立て) では「＿＿＿」+「★」の使用が正規。
12. HTMLタグ (<u>, </u>, <b>, <i>, <span>等) は出力に含めないこと。
    フロントエンドでは JSX のエスケープにより HTMLタグは素のテキストとして表示されるため
    ユーザ画面に <u> などが見えてしまう。下線を引きたい語句は【】で囲むか、
    sub_question の sentence に単純に含めること。
13. N5/N4 (初級) では選択肢は短く揃えること。
    全選択肢の文字数差が 5 以内、最長でも 12 文字以内を目安とする。
    「〜してあります」「〜ているところです」等の複合文法はN3以上なので初級では避けること。

【品質検証要件（実データで繰り返し発見された致命的欠陥）】
14. 正解の一意性を出力前に自己確認すること。
    「から」vs「ので」「ために」vs「のに」のように両方成立する選択肢は禁止。
    正解以外の 3 つについて「なぜ不正解か」を内部で説明できる場合のみ出力。
15. 読解・聴解問題 (category=10-17) では、parent.prerequisites に本文/会話を必ず含め、
    正解は本文から直接導出できるものに限る。本文にない事実を正解にしない。
16. 並び替え問題 (category=9) では、select_answer の value に必ず実際の語句を入れる。
    value を "1"/"2"/"3"/"4" のような数字だけにしない。正解 key の並びで文が成立すること。
17. 漢字読み問題 (category=2) では sub.sentence に必ず読みを問う漢字を含める。
    括弧内は空欄にせず、読み方を問いたい漢字を明示する（例：「わたしの（国）は〜」）。
18. 選択肢の品詞を統一すること。い形容詞とな形容詞の混在、動詞と名詞の混在は禁止。"#;
